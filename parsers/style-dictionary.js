...
const outputFilePath = path.join(__dirname, '../tokens/parsed/style-dictionary/tokens.json');

// Load your original design tokens JSON file
fs.readFile(inputFilePath, 'utf8', (err, data) => {
  if (err) throw err;
  const designTokensJson = JSON.parse(data);
  const modifiedJson = transformDesignTokens(designTokensJson); // No need to modify references for Style Dictionary

  // Write the modified JSON to a new file
  fs.writeFile(outputFilePath, JSON.stringify(modifiedJson, null, 2), (err) => {
    if (err) throw err;
    console.log('Style Dictionary tokens saved!');
  });
});